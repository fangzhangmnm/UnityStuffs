...
// credit https://www.videopoetics.com/tutorials/pixel-perfect-outline-shaders-unity/
Shader "Custom/ToonOutline"
{
    Properties
    {
        ...
        [HDR]_AmbientColor("Ambient Color",Color)=(.4,.4,.4,1)
		...
		_OutlineColor("Outline Color", Color) = (0.25, 0.32, 0.5, 1)
    ...
    SubShader
    {
		...
		Pass {//Outline
			...
			float4 VertexProgram(float4 position : POSITION,float3 normal : NORMAL) : SV_POSITION{
				...
				float2 offset = normalize(clipNormal.xy) / _ScreenParams.xy * _OutlineWidth * clipPosition.w * 2;//can be optimized on cpu
